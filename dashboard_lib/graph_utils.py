...
def create_conditional_style(df):
	...
	for col in df.columns:
		...
		if name_length == 0 or math.isnan(name_length):
			...
			pixel = round(name_length * 0.55) + 1
...
def generate_table_selectable(dataframe: pd.DataFrame, id_table='', max_height='50rem', selectable='multi', filt='none',
	...
	if style_cells is None:
		style_cells = CELL_STYLES
	out = html.Div([html.Div(
		dash_table.DataTable(columns=[{"name": i[0], "id": i[0],
		                               'type': TYPES[i[1]],
		                               'format': FORMATS[i[1]]
		                               }
		                              for i in
		                              [(i, str(row)) if 'time' not in str(row) else (i, 'datetime[ns]') for i, row
		                               in dataframe.dtypes.iteritems()]],

		                     data=dataframe.to_dict('records'),
		                     style_cell={'padding': '1px 8px 1px', 'vertical-align': 'top', 'overflow-y': 'hidden',
		                     ...
		                     style_table={'maxWidth': '100%', 'maxHeight': max_height, 'overflowY': 'auto',
		                                  'overflowX': 'auto', 'font-size': '12px', 'text-align': 'left',
		                                  'padding': '2px 8px 2px'},
...
def gera_fig_pie_chart(labels, values, titulo, textposition='inside', text='', showleg=True, height=None,
                       width=None, bgcolor='#454545', textcolor='white'):
